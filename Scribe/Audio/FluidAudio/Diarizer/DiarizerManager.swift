...
final class DiarizerManager {
    ...
    var initializationTimings: (downloadTime: TimeInterval, compilationTime: TimeInterval) {
        models.map { ($0.downloadDuration, $0.compilationDuration) } ?? (0, 0)
    }

    func initialize(models: consuming DiarizerModels) {
    ...
    /// Accepts FASpeaker structs and adds them to the in-memory database.
    ///
    /// - Parameter speakers: Array of FASpeaker structs with embeddings and metadata
    ...
    /// ```
    func performCompleteDiarization<C>(
