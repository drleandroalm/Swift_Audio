...
enum SpeakerUtilities {
    ...
    struct AssignmentConfig {
        let maxDistanceForAssignment: Float
        let maxDistanceForUpdate: Float
        let minSpeakerDuration: Float
        let minSegmentDuration: Float
        ...
        static let macOS = AssignmentConfig(
        ...
        static let iOS = AssignmentConfig(
        ...
        static var current: AssignmentConfig {
            #if os(macOS)
            return .macOS
            #else
            return .iOS
    ...
    static func validateEmbedding(_ embedding: [Float], minMagnitude: Float = 0.1) -> Bool {
    ...
    // MARK: - FASpeaker Assignment Decision
    ...
    static func shouldAssignSpeaker(
        distance: Float,
        duration: Float,
    ...
    static func findClosestSpeaker(
        embedding: [Float],
        candidates: [FASpeaker]
    ) -> (speaker: FASpeaker?, distance: Float) {
        guard !candidates.isEmpty else {
            return (nil, Float.infinity)
        }

        var minDistance = Float.infinity
        var closestSpeaker: FASpeaker?
    ...
    // MARK: - FASpeaker Creation

    /// Validated speaker creation parameters
    struct SpeakerCreationParams {
        let id: String
        let name: String
        let duration: Float
        let embedding: [Float]
    ...
    /// Create a new FASpeaker instance with validation
    ...
    static func removeRawEmbedding(
    ...
    // MARK: - Complete FASpeaker Update Operations

    /// Complete speaker update operation including raw tracking
    struct SpeakerUpdateResult {
        let updatedMainEmbedding: [Float]?
        let updatedRawEmbeddings: [RawEmbedding]
        let updatedDuration: Float
        let shouldRecalculate: Bool
    }

    /// Updates a speaker with new segment data - handles both main and raw embeddings
    static func updateSpeakerWithSegment(
...
extension SpeakerManager {
    ...
    // MARK: - FASpeaker Query Operations
    ...
    func getCurrentSpeakerNames() -> [String] {
        return queue.sync {
