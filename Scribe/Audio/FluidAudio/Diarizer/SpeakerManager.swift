...
class SpeakerManager {
    ...
    // Constants
    static let embeddingSize = 256  // Standard embedding dimension for speaker models

    // FASpeaker database: ID -> FASpeaker
    internal var speakerDatabase: [String: FASpeaker] = [:]
    ...
    var speakerThreshold: Float  // Max distance for speaker assignment (default: 0.65)
    var embeddingThreshold: Float  // Max distance for updating embeddings (default: 0.45)
    var minSpeechDuration: Float  // Min duration to create speaker (default: 1.0)
    var minEmbeddingUpdateDuration: Float  // Min duration to update embeddings (default: 2.0)
    ...
    func initializeKnownSpeakers(_ speakers: [FASpeaker]) {
    ...
    private func createNewSpeaker(
        ...
        // Create new FASpeaker object
        let newSpeaker = FASpeaker(
            id: newSpeakerId,
    ...
    func getAllSpeakers() -> [String: FASpeaker] {
        queue.sync {
            return speakerDatabase
        }
    }

    func getSpeaker(for speakerId: String) -> FASpeaker? {
        queue.sync { speakerDatabase[speakerId] }
    }

    /// - Parameter speaker: The FASpeaker object to upsert
    func upsertSpeaker(_ speaker: FASpeaker) {
        upsertSpeaker(
            id: speaker.id,
